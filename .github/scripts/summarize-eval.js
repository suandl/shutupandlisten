...
const exitCode = process.env.EVAL_EXIT_CODE || '';
const headSha = process.env.HEAD_SHA || '';
const runUrl = process.env.RUN_URL || '';
const artifactName = process.env.ARTIFACT_NAME || 'promptfoo-report';
...
function fail(note) {
  ...
  out('');
  out(note);
  if (runUrl) out(`\n[Full report artifact (\`${artifactName}\`)](${runUrl})`);
  process.exit(0);
}
...
try {
  ...
  fail(`_Could not read promptfoo results (\`${e.message}\`). See the full report artifact._`);
}
...
if (!rows.length) {
  fail('_No eval cells were parsed from the results file. See the full report artifact._');
}
...
if (headSha) bits.push(`commit \`${headSha.slice(0, 7)}\``);
if (exitCode === '100') bits.push('_some cells below judge threshold_');
out(bits.join(' · '));
out('');
...
// Full breakdown (collapsible): provider × scenario × judge
out('<details>');
...
const footer = [];
if (runUrl) footer.push(`[Full report artifact (\`${artifactName}\`)](${runUrl})`);
